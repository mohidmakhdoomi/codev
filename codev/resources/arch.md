...
- **Graceful stop**: `afx workspace stop` sets the `intentionallyStopping` flag (`tower-instances.ts`) so the six cascaded architect exit handlers (4 in `tower-instances.ts`, 2 in `tower-terminals.ts`) skip BOTH the `setArchitectByName(name, null)` call AND the per-session `deleteTerminalSession(session.id)` call. Additionally, `deleteWorkspaceTerminalSessions` defaults to preserving `type='architect'` rows (`includeArchitects: false`). Together these keep the architect's `workspace_path` signal in `terminal_sessions` alive across stop+start (Bugfix #826).
- **Graceful start**: `launchInstance` creates `main` if absent (gate changed from `entry.architects.size === 0` to `!entry.architects.has('main')`), then iterates workspace-scoped siblings via `getArchitectsForWorkspace(resolvedPath)` — which joins `state.db.architect` with `terminal_sessions.workspace_path` — and re-spawns each via `addArchitect`. Critical ordering: main FIRST (otherwise `addArchitect`'s `size > 0` guard rejects the sibling spawn). Bugfix #826 — the workspace scoping prevents architects registered in workspace A from leaking into workspace B at launch.
...
- **Permanent exit** (max-restart exhaustion): exit handlers run WITHOUT the intentional-stop flag set, so `deleteTerminalSession(session.id)` AND `setArchitectByName(name, null)` BOTH fire and rows are auto-deleted (Spec 786 OQ-B — `state.db` mirrors reality).
- **Stop-all** (`tower-routes.ts:handleWorkspaceStopAll`): explicit "tear everything down" — full wipe of `terminal_sessions` (via `deleteWorkspaceTerminalSessions(..., { includeArchitects: true })`) AND `state.db.architect`. Semantically distinct from `stopInstance` which preserves architect registration.

**Persistence layers**:
- `state.db.architect` — durable per-architect registration (`id, pid, port, cmd, started_at, terminal_id`). `pid`/`port` persist as `0` (Spec 755 limitation); the live values come from Tower's `PtySession` only. Anchored to Tower's startup CWD (no `workspace_path` column — a separate workspace-scoping signal is needed; see `terminal_sessions` below).
- `terminal_sessions` — global runtime session registry. Has a `workspace_path` column that, for `type='architect'` rows, doubles as the workspace-scoping signal that `getArchitectsForWorkspace` joins on (Bugfix #826). Architect rows are PRESERVED on graceful stop and wiped only on stop-all or permanent exit; non-architect rows (builders, shells) are wiped on graceful stop as before. `saveTerminalSession` enforces a `(workspace_path, role_id)` uniqueness invariant for architect rows so repeated stop+start cycles don't accumulate stale rows. Reconciliation reads `role_id` to re-key the in-memory architect map.
