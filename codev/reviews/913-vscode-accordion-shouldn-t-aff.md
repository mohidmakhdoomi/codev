...
- **Synchronous `changeEmitter.fire()` inside `onDidExpandElement`**: `collapseBuildersExcept` fires the tree-data change synchronously from within the expand handler (during `openBuilderRow`'s `reveal`). Verified correct in logic and at the dev-approval gate; the `openBuilderId` guard in `extension.ts` prevents a re-fire loop on the same row.
