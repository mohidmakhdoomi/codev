...
- **The tsdown config is deliberately `.mjs`, not `.ts`.** tsdown loads a `.ts` config via a TypeScript-config loader (`unrun`/`tsx`) that it only declares as an *optional* peer — so a clean `--frozen-lockfile` install (CI) can't load it and the build dies with "Failed to import module 'unrun'". (This actually failed CI on the first PR push and was fixed here.) A plain ESM `.mjs` config is imported natively by Node with no loader, which is portable across local and CI. Verified by rebuilding with the hoisted `tsx` loader hidden. Keep the config `.mjs`.
