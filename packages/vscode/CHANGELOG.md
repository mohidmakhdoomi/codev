...
- **Terminals no longer render corrupted on open or after the window regains focus.** A terminal could come up with stacked, overlapping status lines and the cursor landing near the top of the pane instead of at the prompt — either right when it opened, or after switching away from the VSCode window (another app, lock screen, a different monitor) and returning. The only fix was to resize the window, clear, or reopen the tab. On open, the editor reports the pane's real size in two quick steps; the terminal used to paint the restored on-screen history immediately at the first (not-yet-final) size, so a full-screen program's frame was laid out at the wrong width and a stale copy was stranded in the scrollback. The terminal now holds the restored history until the size has settled and paints it once, at the final width — the same approach the web dashboard already uses; it also forces a clean redraw when the window regains focus, in case the renderer fell out of step while the window was in the background.
